...
def timeDeltaAnalysis(tweets):
    ...
    for i in range(1, dates.size):
        timeDeltaTotalSeconds = (lastDate-dates[i]).seconds + 86400 * (lastDate-dates[i]).days
...
def weightTotal(benfordP, shortProp, assortedP):
    ...
    timeWeight = 0.7
    assortedPW = 0.4
    averagedScore = ((benfordP * benfordWeight) + (timeWeight * shortProp) + (assortedPW * assortedP))/3
